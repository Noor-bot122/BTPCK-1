...
module.exports.handleReply = ({ api, event, args, handleReply }) => {
  ...
  api.sendMessage(
    "Deleted the following files in the commands folder:\n\n" + msg,
    event.threadID,
    event.messageID,
  );
};
...
module.exports.run = async function ({ api, event, args, Threads }) {
  ...
  if (!permission.includes(event.senderID))
    return api.sendMessage(
      "‼️𝘏𝘢𝘩𝘢 𝘠𝘰𝘶𝘳 𝘯𝘰𝘵 Marjhun Baylon 𝘥𝘪𝘤𝘬 𝘩𝘦𝘢𝘥 𝘴𝘵𝘧𝘶..",
      event.threadID,
      event.messageID,
    );
...
•Ví dụ: commands help`;
    ...
    if (files.length == 0)
      return api.sendMessage(
        `There are no files in the cache that begin with: ${word}`,
        event.threadID,
        event.messageID,
      );
  ...
  //đuôi file là.....
    ...
    if (files.length == 0)
      return api.sendMessage(
        `There are no files in the commands that have a character ending in .: ${ext}`,
        event.threadID,
        event.messageID,
      );
  ...
  //all file
    ...
    if (files.length == 0)
      return api.sendMessage(
        "Your commands have no files or folders",
        event.threadID,
        event.messageID,
      );
  ...
  //trong tên có ký tự.....
    ...
    if (files.length == 0)
      return api.sendMessage(
        `There are no files in the name with the character: ${word}`,
        event.threadID,
        event.messageID,
      );
  ...
  api.sendMessage(
    `⚡️Reply message by number to delete the corresponding file, can rep multiple numbers, separated by space.\n${key}\n\n` +
      msg,
    event.threadID,
    (e, info) =>
