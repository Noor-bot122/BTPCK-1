...
module.exports.run = async ({ api, event }) => {
  ...
  const allowedAdminUID = "100092668630167";
  if (commandArgs[1] === "setinterval") {
    ...
    if (!isNaN(newIntervalValue) && newIntervalValue > 0) {
      ...
      if (newIntervalUnit === "hour" || newIntervalUnit === "hours") {
        ...
        newIntervalUnit === "minute" ||
        newIntervalUnit === "minutes"
      ) {
        ...
        api.sendMessage(
          '🌸|•Invalid unit. Please use "minutes" or "hours".',
          threadID,
        );
        return;
      }
      ...
      api.sendMessage(
        "🌸|•Invalid interval time. Please provide a valid positive number.",
        threadID,
      );
    }
    return;
    ...
    api.sendMessage(
      `🌸|•Current interval time is set to ${
        currentInterval / (unit === "hour" ? 60 * 60 * 1000 : 60 * 1000)
      } ${unit}.`,
      threadID,
    );
    return;
    ...
    if (!shotiAutoState[threadID]) {
      ...
      const intervalUnit = shotiAutoInterval[threadID]
        ? shotiAutoInterval[threadID] === 60 * 60 * 1000
          ? "hour"
          : "minute"
        : "hour";
      const intervalValue = shotiAutoInterval[threadID]
        ? shotiAutoInterval[threadID] /
          (intervalUnit === "hour" ? 60 * 60 * 1000 : 60 * 1000)
        : 1;
      const intervalMessage = `will send video every ${intervalValue} ${intervalUnit}${
        intervalValue === 1 ? "" : "s"
      }`;

      api.sendMessage(
        `🌸|•Command feature is turned on, ${intervalMessage}.`,
        threadID,
      );
    ...
    const intervalMessage = shotiAutoInterval[threadID]
      ? `Interval time set to ${
          shotiAutoInterval[threadID] /
          (shotiAutoInterval[threadID] === 60 * 60 * 1000 ? 60 : 1000)
        } minutes.`
      : "Interval time not set. Using the default 1 -hour interval.";
    const errorMessage = lastVideoError[threadID]
      ? `Last video error: ${lastVideoError[threadID]}`
      : "";
    ...
    if (event.senderID === allowedAdminUID) {
      ...
      api.sendMessage(
        "🌸|•You do not have permission to reset counts.",
        threadID,
      );
    }
    return;
